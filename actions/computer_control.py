...
def _click(x=None, y=None, button: str = "left", clicks: int = 1) -> str:
    ...
    if x is not None and y is not None:
        _natural_move(int(x), int(y), duration=0.08)
        pyautogui.click(x, y, button=button, clicks=clicks)
        return f"{'Double-c' if clicks == 2 else 'C'}licked ({x}, {y}) [{button}]"
    time.sleep(0.02 + random.uniform(0.01, 0.06))
...
def _natural_move(x: int, y: int, duration: float = 0.35) -> None:
    _require_pyautogui()
    ...
    total = max(0.035, min(float(duration), 0.18, 0.035 + distance / 6000.0))
    steps = max(4, min(14, int(distance / 100) + 4))
    for i in range(1, steps + 1):
        t = i / steps
        # Smoothstep easing keeps the movement responsive but not robotic.
        eased = t * t * (3 - 2 * t)
        px = round(start_x + (x - start_x) * eased)
        py = round(start_y + (y - start_y) * eased)
        pyautogui.moveTo(px, py, duration=total / steps)


def _move(x: int, y: int, duration: float = 0.3) -> str:
    _require_pyautogui()
    _natural_move(x, y, duration=duration)
    return f"Mouse → ({x}, {y})"
...
def _uia_find(description: str) -> tuple[int, int] | None:
    """Fast native Windows UIA lookup before invoking vision."""
    if not _UIA:
        return None
    try:
        ...
        wanted = [w for w in re.findall(r"[a-z0-9]+", description.lower()) if len(w) > 2]
        if not wanted:
            return None
        best = None
        best_score = 0
        for control in root.descendants():
            try:
                ...
                hay = " ".join(str(v or "") for v in (
                    control.window_text(), info.name, info.automation_id,
                    info.control_type, info.class_name,
                )).lower()
                if not hay:
                    continue
                score = sum(1 for word in wanted if word in hay)
                if score == 0:
                    continue
                rect = control.rectangle()
                if rect.width() < 5 or rect.height() < 5:
                    continue
                # UIA can expose stale/off-screen rectangles from closed or
                # minimized windows. Never pass those coordinates to PyAutoGUI.
                try:
                    screen_w, screen_h = pyautogui.size()
                    if rect.right <= 0 or rect.bottom <= 0 or rect.left >= screen_w or rect.top >= screen_h:
                        continue
                except Exception:
                    pass
                if score > best_score:
                ...
                continue
        if best is not None and best_score >= max(1, len(wanted) // 2):
            return int((best.left + best.right) / 2), int((best.top + best.bottom) / 2)
    except Exception:
        pass
    return None


def _local_screen_find(description: str) -> tuple[int, int] | None:
    """Use the bundled VLM as a private fallback for icon/text targets."""
    _require_pyautogui()
    img = pyautogui.screenshot()
    buf = io.BytesIO()
    ...
    w, h = img.size
    prompt = (
        f"Find the visible desktop control described as {description!r} in this {w}x{h} screenshot. "
        'Return ONLY JSON {"found":true,"x":0,"y":0,"confidence":0.0}. '
        "Use the center of the clickable control, or found:false if absent."
    )
    data = json.loads(local_vision.analyze(buf.getvalue(), prompt, max_tokens=64))
    x, y = int(data.get("x", 0)), int(data.get("y", 0))
    if data.get("found") and float(data.get("confidence", 0)) >= 0.35 and 0 <= x < w and 0 <= y < h:
        return x, y
    return None
...
def _screen_find(description: str) -> tuple[int, int] | None:
    ...
    if native:
        print(f"[ComputerControl] UIA fast match: {native}")
        return native

    # Local-only fallback. Reusable automation must not depend on a paid or
    # rate-limited cloud vision request to find a native desktop control.
    try:
        coords = _local_screen_find(description)
        if coords:
            print(f"[ComputerControl] Local vision match: {coords}")
            return coords
    except Exception as local_error:
        print(f"[ComputerControl] Local vision unavailable: {local_error}")
...
def computer_control(
    ...
    try:
        ...
        if action in ("screen_click", "smart_click"):
            ...
            coords = None
            # A fresh screenshot per attempt handles UI animations and delayed
            # app rendering without blindly clicking the old coordinate.
            for attempt in range(2):
                ...
                if attempt == 0:
                    time.sleep(0.12)
            if not coords:
                return f"Could not locate '{desc}' on screen."
            return _click(coords[0], coords[1], params.get("button", "left"), 1)
