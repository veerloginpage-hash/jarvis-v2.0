...
def _capture_screen() -> tuple[bytes, str]:
    ...
    future = _EXECUTOR.submit(_compress, png, "PNG")
...
def _capture_camera() -> tuple[bytes, str]:
    ...
    if _PIL:
        ...
        future = _EXECUTOR.submit(_compress, png_bytes, "PNG")
        return future.result(timeout=20)
...
def screen_process(
    ...
    # Local SmolVLM is the low-latency primary path. Remote Gemini remains a
    # fallback for installations where llama.cpp is unavailable or fails.
    try:
        result = local_vision.analyze(image_bytes, user_text)
        if result:
            if player:
                player.write_log(f"Jarvis: {result}")
            return f"Screen Analysis:\n{result}"
    except Exception as e:
        print(f"[Vision] Local SmolVLM unavailable: {e}")

    try:
        _ensure_session(player=player)
    except Exception as e:
        print(f"[Vision] ❌ Could not start fallback session: {e}")
        return False
...
def warmup_local() -> None:
    """Best-effort background warmup; never blocks Jarvis startup."""
    if local_vision.is_available():
        threading.Thread(target=local_vision.warmup, daemon=True, name="LocalVisionWarmup").start()
