...
def list_windows() -> str:
    if _OS == "Windows":
        try:
            ...
            return ", ".join(titles) if titles else "No windows found."
        except Exception as e:
        ...
        try:
            ...
            return f"list_windows failed: {e}"
    return "list_windows not supported on this OS."
...
def focus_window(title: str) -> str:
    ...
    if _OS == "Windows":
        ...
        try:
            ...
            return f"focus_window failed: {e}"
    return "focus_window not supported on this OS."
...
def close_window_by_title(title: str) -> str:
    ...
    if _OS == "Windows":
        try:
            ...
            return f"close_window_by_title failed: {e}"
    elif _OS == "Linux":
        try:
            ...
            return f"close_window_by_title failed: {e}"
    return "close_window_by_title not supported on this OS."
...
def resize_window(width: int, height: int) -> str:
    if _OS == "Windows":
        ...
        try:
            subprocess.run(["wmctrl", "-r", ":ACTIVE:", "-e", f"0,-1,-1,{width},{height}"],
                capture_output=True)
            return f"Resized active window to {width}x{height}."
        except Exception as e:
            return f"resize_window failed: {e}"
    return "resize_window not supported on this OS."
...
def move_window(x: int, y: int) -> str:
    if _OS == "Windows":
        ...
        try:
            subprocess.run(["wmctrl", "-r", ":ACTIVE:", "-e", f"0,{x},{y},-1,-1"],
                capture_output=True)
            return f"Moved active window to ({x}, {y})."
        except Exception as e:
            return f"move_window failed: {e}"
    return "move_window not supported on this OS."
...
def computer_settings(
    ...
    if action in ("focus_window", "switch_to_window", "bring_window"):
        title = str(value or params.get("title", "") or params.get("description", "")).strip()
        return focus_window(title)

    if action in ("close_window_by_title", "close_specific_window"):
        title = str(value or params.get("title", "") or params.get("description", "")).strip()
        return close_window_by_title(title)

    if action in ("resize_window", "window_resize"):
        try:
            w = int(params.get("width", value or 800))
            h = int(params.get("height", 600))
            return resize_window(w, h)
        except Exception as e:
            return f"resize_window failed: {e}"

    if action in ("move_window", "window_move"):
        try:
