...
from core.task_runtime import checkpoint, clear_checkpoint, load_checkpoint, record

STEP_TIMEOUT_SECONDS = 90
_STEP_POOL = ThreadPoolExecutor(max_workers=4, thread_name_prefix="JarvisStep")
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    if tool == "open_app":
        ...
        return automation(parameters=parameters, player=None) or "Done."

    elif tool == "local_workflow":
        from actions.local_workflow import local_workflow
        return local_workflow(parameters=parameters, player=None) or "Done."
...
def _tool_failed(result: object) -> bool:
    """Treat explicit failure responses as failures, not false successes."""
    text = str(result or "").lower()
    failure_markers = (
        "could not", "can't", "cannot", "failed", "failure", "error:",
        "not found", "timed out", "timeout", "no action", "unable to",
    )
...
class AgentExecutor:
    ...
    def execute(
        ...
        print(f"\n[Executor] 🎯 Goal: {goal}")
        runtime_id = runtime_id or f"task-{abs(hash((goal, time.time_ns()))) % 10**10:010d}"
        record(runtime_id, "started", goal=goal[:500])
        ...
        # Resume durable work after an unexpected process/network failure.
        # The runtime id is stable for queued tasks, so only that task's own
        # checkpoint can be used here.
        saved = load_checkpoint(runtime_id)
        resumed_ids = set(saved.get("completed_steps", [])) if saved else set()
        if resumed_ids:
            completed_steps = [
                step for step in plan.get("steps", [])
                if step.get("step") in resumed_ids
            ]
            if saved.get("last_result") and completed_steps:
                step_results[completed_steps[-1].get("step")] = saved["last_result"]
            record(runtime_id, "resumed", completed_steps=sorted(resumed_ids))
            print(f"[Executor] ♻️ Resuming from completed steps: {sorted(resumed_ids)}")
        ...
        while True:
            ...
            for step in steps:
                if step.get("step") in resumed_ids:
                    print(f"[Executor] ⏭️ Step {step.get('step')} already completed; skipping")
                    continue
                ...
                while attempt <= 3:
                    if cancel_flag and cancel_flag.is_set():
                        break
                    try:
                        record(runtime_id, "step_started", step=step_num, tool=tool)
                        future = _STEP_POOL.submit(_call_tool, tool, params, speak)
                        try:
                            result = future.result(timeout=STEP_TIMEOUT_SECONDS)
                        except FutureTimeout as exc:
                            future.cancel()
                            raise TimeoutError(
                                f"Step {step_num} timed out after {STEP_TIMEOUT_SECONDS}s"
                            ) from exc
                        ...
                        record(runtime_id, "step_verified", step=step_num, tool=tool,
            ...
            if success:
                result = self._summarize(goal, completed_steps, speak)
                record(runtime_id, "completed", result=result[:500])
                clear_checkpoint(runtime_id)
                return result

            if replan_attempts >= self.MAX_REPLAN_ATTEMPTS:
                msg = f"Task failed after {replan_attempts} replan attempts, sir."
                if speak: speak(msg)
                record(runtime_id, "failed", result=msg)
                return msg
    ...
    def _summarize(self, goal: str, completed_steps: list, speak: Callable | None) -> str:
        ...
        # A second LLM call adds noticeable latency to short commands and does
        # not improve their result. Keep the fast path genuinely fast.
